...
export default function BottomNav() {
  ...
  const navItems = [
    ...
    { to: "/notifications", icon: Bell, badge: unreadCount > 0 ? unreadCount : null },
      ...
      {navItems.map(({ to, icon: Icon, badge, isAvatar }) => {
        ...
        return (
          ...
          >
            ...
            {badge && (
              <span className="absolute top-1.5 right-[calc(50%-14px)] min-w-[16px] h-4 bg-blue-600 text-white text-[9px] font-bold rounded-full flex items-center justify-center px-1">
                {badge > 9 ? "9+" : badge}
              </span>
            )}
          </button>
